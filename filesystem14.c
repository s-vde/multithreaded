...
/**
 ...
 @details . In the paper, NUM_BLOCKS = 26 and NUM_INODE = 32.
...
void *thread(void* arg)
{
    ...
    // If the inode does not already have an associated block, search for a free
	// block to allocate to the inode.
    if (inode[i] == 0) {
        // Search starts at an arbitrary block index to avoid excessive lock
		// contention.
